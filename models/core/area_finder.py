...
class SpatialRegionGenerator:
    ...
    def draw_distance_region(self, img, center, abs_angle, cmp, value_px,
                             tolerance=15, spread_deg=40, color=(0,255,255)):
        cx, cy = center
        theta_left = np.deg2rad(abs_angle - spread_deg / 2)
        theta_right = np.deg2rad(abs_angle + spread_deg / 2)
        max_radius = int(np.sqrt(self.W**2 + self.H**2))
        ...
        if cmp == "eq":
            ...
            r1 = max(0, value_px - tolerance)
            r2 = value_px + tolerance
    ...
    def visualize_simple(self):
        ...
        for i, constraint in enumerate(self.query.get("constraints", [])):
            ...
            dist_dir  = constraint.get("distance", {}).get("direction")
            cmp       = constraint.get("distance", {}).get("cmp", "eq")
            val_px    = constraint.get("distance", {}).get("value_px")
            tol       = constraint.get("distance", {}).get("tolerance_px", 15)
            ...
            if abs_angle is not None and val_px is None:
                ...
                self.draw_distance_region(output, obj["center"], abs_angle, cmp, val_px, tol, 40, color)
